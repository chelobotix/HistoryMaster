module Auth
  module Google
    class AuthUrlGenerator
      ...
      GOOGLE_OAUTH2_URL = ENV["GOOGLE_OAUTH2_URL"].freeze
      private_constant :GOOGLE_OAUTH2_URL
      ...
      # @return [void]
      def generate_url
        @url = "#{GOOGLE_OAUTH2_URL}?state=#{state_token}&client_id=#{GOOGLE_CLIENT_ID}&redirect_uri=#{GOOGLE_REDIRECT_URL}&response_type=code&scope=email profile"
      end
    end
  end
end
